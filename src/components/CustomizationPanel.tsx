  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
      ...
      <button
        ...
        className="w-full px-5 sm:px-6 py-4 flex items-center justify-between hover:bg-slate-50/70 transition-colors text-left"
      >
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h2 className="text-sm font-semibold text-slate-800">Customize QR Design & Quality</h2>
            <p className="text-xs text-slate-400">Colors, error correction, and export size</p>
          </div>
        </div>
      ...
      {isOpen && (
        <div className="px-5 sm:px-6 pb-6 pt-2 space-y-5 border-t border-slate-100">
          {/* Colors */}
          <div>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 mb-3">
              {COLOR_PRESETS.map((preset) => {
                ...
                return (
                  ...
                  >
                    <div
                      className="w-6 h-6 rounded-lg border border-black/10 shadow-2xs flex items-center justify-center overflow-hidden"
                    ...
                    <span className="text-[11px] text-slate-600 truncate max-w-full font-medium">
                      {preset.name}
                    </span>
                  </button>
                );
            ...
            <div className="grid grid-cols-2 gap-3 pt-1">
              <div className="flex items-center gap-2 p-2 rounded-xl border border-slate-200 bg-slate-50/50">
                ...
                <div className="flex-1 min-w-0">
                  <span className="block text-[10px] text-slate-400 font-medium uppercase">Foreground</span>
                  <span className="block text-xs font-mono text-slate-700">{options.fgColor}</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 p-2 rounded-xl border border-slate-200 bg-slate-50/50">
                ...
                />
                <div className="flex-1 min-w-0">
                  <span className="block text-[10px] text-slate-400 font-medium uppercase">Background</span>
                  <span className="block text-xs font-mono text-slate-700">{options.bgColor}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2 border-t border-slate-100">
            {/* Error Correction */}
            <div>
              <label className="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-2 flex items-center gap-1.5">
